...
* **金额默认含税、脚本自动换算**：客户提供的金额**默认为含税金额**（无需追问是否含税）。脚本自动将含税金额换算为不含税金额(`amount`)、不含税单价(`unitPrice`)、税额(`taxAmount`)。税率自动转为小数形式（如 `6%` 转为 `0.06`）
...
| amount | ✅ | **客户提供的含税金额（元）**。脚本自动换算为不含税金额、不含税单价、税额 |
| taxRate | ✅ | 税率，如 `6%`、`1%`、`13%`。脚本自动转为小数形式 |
