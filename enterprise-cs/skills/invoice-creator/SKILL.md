...
* **含税金额与税率规范（OpenAPI v1.1）**：客户提供的金额填入 `taxInclusiveAmount`（含税金额/价税合计），`taxRate` 传字符串形式（如 `"0.06"`、`"0.01"`，脚本自动转化归一化）。后端不再接收 `unitPrice`、`amount`、`taxAmount` 字段，含税单价由后端按 `含税金额 / 数量` 自动计算
